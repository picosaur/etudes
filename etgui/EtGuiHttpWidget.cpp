...
public:
  ...
  HttpHeadersList(const EtHttp::HttpHeaders &emHeaders) {
  ...
  void setHeaders(const EtHttp::HttpHeaders &emHeaders) {
...
public:
  ...
  EtHttp::HttpHeaders getReqHeaders() const;
};
...
void HttpWidget::Impl::showRequest() {
  ...
  if (ImGui::Button("Fetch")) {
    resp = {};
    fetcher = std::make_unique<EtHttp::HttpFetcher>(url, getReqHeaders());
  }
  showReqHeadersTable();
}
...
void HttpWidget::Impl::setBrowserReqHeaders(const std::string &url) {
  ...
  reqHeaders.setHeaders(EtHttp::BrowserRequestHeaders(EtHttp::UrlHost(url)));
}

void HttpWidget::Impl::setStandardReqHeaders() {
  reqHeaders.setHeaders(EtHttp::StandardRequestHeaders());
}

EtHttp::HttpHeaders HttpWidget::Impl::getReqHeaders() const {
  EtHttp::HttpHeaders emHeaders;
